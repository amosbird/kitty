...
glfw_xkb_release(_GLFWXKBData *xkb) {
    ...
    glfw_fcitx5_terminate(&xkb->fcitx5);
...
glfw_xkb_create_context(_GLFWXKBData *xkb) {
    ...
    glfw_connect_to_fcitx5(&xkb->fcitx5);
    if (!xkb->fcitx5.inited) glfw_connect_to_ibus(&xkb->ibus);
#endif
    return true;
}
...
glfw_xkb_update_ime_state(_GLFWwindow *w, _GLFWXKBData *xkb, const GLFWIMEUpdateEvent *ev) {
    ...
    switch(ev->type) {
        case GLFW_IME_UPDATE_FOCUS:
            if (xkb->fcitx5.ok)
                glfw_fcitx5_set_focused(&xkb->fcitx5, ev->focused);
            else
        ...
        case GLFW_IME_UPDATE_CURSOR_POSITION:
            ...
            if (xkb->fcitx5.ok)
                glfw_fcitx5_set_cursor_geometry(&xkb->fcitx5, x, y, ev->cursor.width, ev->cursor.height);
            else
...
void
glfw_xkb_forwarded_key_from_ime(xkb_keysym_t keysym, unsigned int glfw_mods, int action) {
    _GLFWwindow *w = _glfwFocusedWindow();
    if (w && w->callbacks.keyboard) {
        GLFWkeyevent fake_ev = {.action = action};
...
glfw_xkb_handle_key_event(_GLFWwindow *window, _GLFWXKBData *xkb, xkb_keycode_t xkb_keycode, int action) {
    ...
    if (xkb->fcitx5.ok) {
        ...
        fcitx5_ev.keycode = ibus_keycode;
