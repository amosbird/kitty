...
scroll_event(const GLFWScrollEvent *ev) {
    ...
    // When scroll_mode_mouse is active, discard momentum (inertia) scroll events
    if (OPT(scroll_mode_mouse) && ev->momentum_type != GLFW_NO_MOMENTUM_DATA
            && ev->momentum_type != GLFW_MOMENTUM_PHASE_BEGAN) return;
    if (ev->y_offset != 0.0) {
        // Scroll mode: use line-based scroll, bypass pixel scroll
        if (screen->scroll_mode.active || (OPT(scroll_mode_mouse) && screen->linebuf == screen->main_linebuf && !screen->modes.mouse_tracking_mode)) {
