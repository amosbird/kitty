...
_PROMPT_PATTERN = re.compile(
    r'(?:^|\s)❯[\s\x00]*$'      # starship, pure, spaceship
    r'|(?:^|\s)➜[\s\x00]*$'     # robbyrussell (oh-my-zsh default)
    r'|(?:^|\s)⟩[\s\x00]*$'     # some minimal prompts
    r'|(?:^|\s)λ[\s\x00]*$'     # haskell-style prompts
    r'|:\)[\s\x00]*$'            # smiley prompt: "hostname :) "
    r'|(?<!\d)[\$#][\s\x00]*$'   # line ending with $ # (not preceded by digit)
    r'|(?<!\d)%[\s\x00]*$'       # line ending with % (not preceded by digit, avoids "100%")
    r'|(?<![=\->])>[\s\x00]*$'   # line ending with > (not after =, -, or > to avoid =>, ->, >>)
    r'|>>>\s'                     # python REPL
    r'|In\s*\[\d+\]'             # IPython/Jupyter
)

_PROMPT_KIND_PROMPT = 1  # PROMPT_START from data-types.h


def _is_likely_progress_bar(text: str) -> bool:
    s = text.rstrip()
...
class ScrollMode:
    ...
    def __init__(self) -> None:
        ...
        self._cached_matches: Optional[List[Tuple[int, int]]] = None
        self._cached_matches_query = ''
        # Selection state
    ...
    def _line_has_prompt_mark(self, abs_line: int) -> bool:
        """Check if the line has an OSC 133 prompt mark (PROMPT_START)."""
        try:
            return self._get_line_obj(abs_line).prompt_kind == _PROMPT_KIND_PROMPT
        except (IndexError, Exception):
            return False

    def _line_matches_prompt_regex(self, abs_line: int) -> bool:
        """Check if the line matches the prompt regex heuristic."""
        try:
            text = str(self._get_line_obj(abs_line))
            if _is_likely_progress_bar(text):
                return False
            return bool(_PROMPT_PATTERN.search(text))
        except (IndexError, Exception):
            return False
    ...
    def enter(self, window: 'Window', silent: bool = False) -> None:
        ...
        If already active, this is a no-op to avoid resetting state.
        """
        if self.active:
            return
    ...
    def enter_prompt_jump(self, window: 'Window') -> None:
        ...
        found_line = -1
        self._window = window  # temporarily set for prompt detection
        self._is_alt_screen = False  # main screen (alt screen returned above)
        for abs_line in range(real_cursor_abs - 1, -1, -1):
            if self._line_has_prompt_mark(abs_line):
                found_line = abs_line
                break
        if found_line < 0:
            for abs_line in range(real_cursor_abs - 1, -1, -1):
                if self._line_matches_prompt_regex(abs_line):
                    found_line = abs_line
                    break
        self._window = None
        if found_line < 0:
    ...
    def _sync_cursor(self) -> None:
        ...
        # Determine cursor width (2 for wide chars, tab width for tabs)
        cw = 1
        try:
            text, widths = self._get_line_text_and_widths(self._cursor_abs)
            ci = self._cell_to_char_idx_w(text, widths, self._cursor_x)
            if ci < len(text):
                cw = widths[ci] if ci < len(widths) else 1
        except (IndexError, Exception):
            pass
    ...
    def _sync_selection(self) -> None:
        ...
        # For line mode, expand to logical line boundaries
        if self._sel_mode == 'line':
            start_abs, _ = self._logical_line_range(start_abs)
            _, end_abs = self._logical_line_range(end_abs)
        ...
        elif sel_type == 1:  # char: extend end_x to cover full wide/tab char
            try:
                end_line, end_widths = self._get_line_text_and_widths(end_abs)
                ci = self._cell_to_char_idx_w(end_line, end_widths, end_x)
                if ci < len(end_line):
                    w = end_widths[ci] if ci < len(end_widths) else 1
                    if w > 1:
                        end_x += w - 1
            except (IndexError, Exception):
                pass
    ...
    def _move_cursor(self, dy: int, dx: int = 0) -> None:
        ...
        if dx > 0 and snapped <= old_x:
            ...
            try:
                text, widths = self._get_line_text_and_widths(self._cursor_abs)
                ci = self._cell_to_char_idx_w(text, widths, old_x)
                if ci < len(text):
                    snapped = old_x + (widths[ci] if ci < len(widths) else 1)
            except (IndexError, Exception):
                snapped = old_x + 1
    ...
    def _find_all_matches(self) -> List[Tuple[int, int]]:
        """Return all (abs_line, cell_column) positions matching the search query.

        Results are cached and only recomputed when the search query changes.
        """
        if self._window is None or not self._search_query:
            return []
        if self._cached_matches is not None and self._cached_matches_query == self._search_query:
        ...
        for abs_line in range(total):
            try:
                line_text, line_widths = self._get_line_text_and_widths(abs_line)
                ...
                while True:
                    ...
                    if col < 0:
                        break
                    matches.append((abs_line, self._char_idx_to_cell_w(line_text, line_widths, col)))
                    start = col + 1
            except (IndexError, Exception):
                continue
        self._cached_matches = matches
        self._cached_matches_query = self._search_query
        return matches
    ...
    def _jump_to_nearest_match(self) -> None:
        ...
        query_lower = self._search_query.lower()
        try:
            line_text, line_widths = self._get_line_text_and_widths(self._cursor_abs)
            line_lower = line_text.lower()
            # Convert cursor cell position to char index for searching
            ci = self._cell_to_char_idx_w(line_text, line_widths, self._cursor_x)
            col = line_lower.find(query_lower, ci)
            if col == ci:
                return  # already on a match
            if col >= 0:
                self._cursor_x = self._char_idx_to_cell_w(line_text, line_widths, col)
                self._sync_cursor()
                return
        except (IndexError, Exception):
            pass
    ...
    def _jump_to_match(self, backwards: bool) -> None:
        ...
        if backwards:
            ...
            try:
                line_text, line_widths = self._get_line_text_and_widths(cur)
                ci = self._cell_to_char_idx_w(line_text, line_widths, cur_x)
                col = line_text.lower().rfind(query_lower, 0, ci)
                if col >= 0:
                    self._cursor_x = self._char_idx_to_cell_w(line_text, line_widths, col)
                    self._sync_cursor()
                    self._mark_dirty()
            ...
            for abs_line in range(cur - 1, -1, -1):
                try:
                    ...
                    if col >= 0:
                        self._move_cursor_to(abs_line, self._char_idx_to_cell_w(line_text, line_widths, col))
                        return
                except (IndexError, Exception):
                    continue
            # Wrap from bottom
            ...
            try:
                line_text, line_widths = self._get_line_text_and_widths(cur)
                ci = self._cell_to_char_idx_w(line_text, line_widths, cur_x)
                col = line_text.lower().find(query_lower, ci + 1)
                if col >= 0:
                    self._cursor_x = self._char_idx_to_cell_w(line_text, line_widths, col)
                    self._sync_cursor()
                    self._mark_dirty()
    ...
    def _jump_to_prompt(self, backwards: bool) -> None:
        ...
        Tries OSC 133 prompt marks first; falls back to regex heuristic.
        """
        if self._window is None:
            return
        total = self._total_lines
        ...
        for checker in (self._line_has_prompt_mark, self._line_matches_prompt_regex):
            if backwards:
                for abs_line in range(cur - 1, -1, -1):
                    if checker(abs_line):
    ...
    def _start_selection(self, mode: str) -> None:
        ...
        self.state = ScrollModeState.SELECT
        if mode == 'line':
            # Expand anchor and cursor to logical line boundaries
            first, last = self._logical_line_range(self._cursor_abs)
            self._sel_start_abs = first
            self._cursor_abs = last
            self._ensure_cursor_visible()
        self._sync_cursor()
        self._mark_dirty()
    ...
    def _handle_search(self, ev: KeyEvent) -> bool:
        ...
        # Printable character(s): append to query (supports IME multi-char input)
    ...
    def _handle_select(self, ev: KeyEvent) -> bool:
        ...
        # q: exit scroll mode entirely (no copy)
        if key == ord('q') and not mods:
            self.exit()
            return True

        # y/Y: yank selection and return to navigate mode (stay in scroll mode)
        if key == ord('y') and mods in (0, GLFW_MOD_SHIFT):
        ...
        if key == ord('v') and mods == GLFW_MOD_SHIFT:
            if self._sel_mode == 'line':
                ...
                self._sel_mode = 'line'
                # Expand anchor and cursor to logical line boundaries
                first_a, last_a = self._logical_line_range(self._sel_start_abs)
                first_c, last_c = self._logical_line_range(self._cursor_abs)
                if self._sel_start_abs <= self._cursor_abs:
                    self._sel_start_abs = first_a
                    self._cursor_abs = last_c
                else:
                    self._sel_start_abs = last_a
                    self._cursor_abs = first_c
                self._ensure_cursor_visible()
            self._sync_cursor()
            self._mark_dirty()
            return True
    ...
    def _line_cells(self, abs_line: int, cols: int) -> list[tuple[str, int]]:
        """Return list of (char, cell_x) pairs for a line, respecting wide chars and tabs."""
        text, widths = self._get_line_text_and_widths(abs_line)
        ...
        for idx, ch in enumerate(text):
            ...
            cell += widths[idx] if idx < len(widths) else 1
        return result
    ...
    def _get_line_obj(self, abs_line: int) -> object:
        """Get the Line C object for an absolute line."""
        if self._window is None:
            raise IndexError('no window')
        screen = self._window.screen
        if self._is_alt_screen:
            return screen.linebuf.line(abs_line)
        h_count = screen.historybuf.count
        if abs_line < h_count:
            return screen.historybuf.line(h_count - 1 - abs_line)
        else:
            return screen.linebuf.line(abs_line - h_count)

    def _get_line_text(self, abs_line: int) -> str:
        """Get the text content of an absolute line (0 = oldest history line)."""
        if self._window is None:
            return ''
        return str(self._get_line_obj(abs_line))

    def _get_line_text_and_widths(self, abs_line: int) -> tuple[str, list[int]]:
        """Get line text and per-character cell widths from the Line object.

        Returns (text, widths) where text == str(line_obj) and widths[i]
        is the number of terminal cells that text[i] occupies.  Tab widths
        are read directly from the C-level Line data (stored as the second
        char in a tab cell's text_cache entry) so they are exact.
        """
        import unicodedata
        line_obj = self._get_line_obj(abs_line)
        text = str(line_obj)
        if '\t' not in text:
            # Fast path: no tabs, compute widths from Unicode properties
            widths = [2 if unicodedata.east_asian_width(ch) in ('W', 'F') else 1 for ch in text]
            return text, widths

        # Slow path: walk cells to get exact tab widths.
        # line_obj[x] for a tab cell returns "\t" + chr(diff) where
        # diff is the total number of cells the tab occupies
        # (i.e. diff = next_tab_stop - cursor_x).
        cols = len(line_obj)  # type: ignore[arg-type]
        widths = []
        x = 0
        for ch in text:
            if ch == '\t' and x < cols:
                cell_text = line_obj[x]  # type: ignore[index]
                if len(cell_text) >= 2 and cell_text[0] == '\t':
                    tab_width = ord(cell_text[1])
                else:
                    # Fallback: shouldn't happen, but use tab-stop formula
                    cell_col = sum(widths)
                    tab_width = 8 - (cell_col % 8) if cell_col % 8 else 8
    ...
    @staticmethod
    def _cell_to_char_idx_w(text: str, widths: list[int], cell_x: int) -> int:
        """Convert a cell (column) position to a Python string index.

        Uses the precomputed widths list for exact cell-width accounting.
        """
        cell = 0
    ...
    @staticmethod
    def _char_idx_to_cell_w(text: str, widths: list[int], char_idx: int) -> int:
        """Convert a Python string index to a cell (column) position.

        Uses the precomputed widths list for exact cell-width accounting.
        """
        cell = 0
        for idx in range(min(char_idx, len(text))):
            cell += widths[idx] if idx < len(widths) else 1
        return cell
    ...
    def _snap_cell_x(self, abs_line: int, cell_x: int) -> int:
        ...
        cell = 0
        for idx, ch in enumerate(text):
            w = widths[idx] if idx < len(widths) else 1
            if cell_x < cell + w:
                return cell
            cell += w
        return cell_x
    ...
    def _is_line_continued(self, abs_line: int) -> bool:
        ...
        h_count = screen.historybuf.count
        if abs_line < h_count:
            return bool(screen.historybuf.is_continued(abs_line))
        else:
            lb_idx = abs_line - h_count
            if lb_idx == 0:
                # First linebuf line: check if history ends with wrap
                if h_count > 0:
                    return bool(screen.historybuf.endswith_wrap())
                return False
            return bool(screen.linebuf.is_continued(lb_idx))

    def _logical_line_range(self, abs_line: int) -> Tuple[int, int]:
        """Return (first_row, last_row) of the logical line containing abs_line.

        A logical line is a sequence of terminal rows connected by soft wrapping.
        Walks backward from abs_line while _is_line_continued() is True to find
        the first row, then walks forward while the next row is continued.
        """
        total = self._total_lines
        first = abs_line
        while first > 0 and self._is_line_continued(first):
            first -= 1
        last = abs_line
        while last + 1 < total and self._is_line_continued(last + 1):
            last += 1
        return first, last
    ...
    def _get_selected_text(self) -> str:
        ...
        if self._sel_mode == 'block':
            ...
            for i in range(start_abs, end_abs + 1):
                try:
                    line, line_widths = self._get_line_text_and_widths(i)
                    ci_left = self._cell_to_char_idx_w(line, line_widths, x_left)
                    ci_right = self._cell_to_char_idx_w(line, line_widths, x_right + 1)
            ...
            try:
                ...
                if self._sel_mode == 'char':
                    if i == start_abs and i == end_abs:
                        ci_s = self._cell_to_char_idx_w(line, line_widths, start_x)
                        ci_e = self._cell_to_char_idx_w(line, line_widths, end_x + 1)
                        line = line[ci_s:ci_e]
                    elif i == start_abs:
                        ci_s = self._cell_to_char_idx_w(line, line_widths, start_x)
                        line = line[ci_s:]
    ...
    def _yank_selection(self, stay: bool = False) -> None:
        """Copy selection to clipboard and return to navigate mode (or exit)."""
