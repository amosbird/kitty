...
class ScrollMode:
    ...
    def enter(self, window: 'Window', silent: bool = False) -> None:
        ...
        # Place scroll mode cursor at the terminal cursor's line
    ...
    def _char_class(self, ch: str) -> int:
        ...
        Returns 0 for whitespace, 1 for word chars (alnum/_), 2 for punctuation.
        """
        if ch.isspace():
            return 0
        ...
        if self._sel_mode == 'block':
            ...
            for i in range(start_abs, end_abs + 1):
                try:
                    ...
                    lines.append(line[ci_left:ci_right].rstrip())
                except (IndexError, Exception):
                    continue
            return '\n'.join(lines)
